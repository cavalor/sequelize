...
class ConnectionManager extends AbstractConnectionManager {
  ...
  connect(config) {
    ...
    /**
     * It is possible that connect event is never fired. Acquire call must always resolve
     * otherwise generic-pool will keep firing TimeoutError
     *
     * So we throw Timeout if we never saw "connect" in set pool.acquire time
     */
      .timeout(this.config.pool.acquire);
  }
